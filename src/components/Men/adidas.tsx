      ...
      <div className="lg:px-10 sm:px-5 pb-6 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-3">
        {
          products.map((product) =>
            <div className=" bg-white overflow-hidden drop-shadow-lg border hover:border-black" key={product.id}>
              <img className="w-full h-[400px] hover:opacity-75 hover:cursor-pointer " src={product.img_url} alt="adidas" />
              ...
              <div className="justify-center flex divide-x-2 divide-gray-400 ">
                ...
                <Link to='/view'><button className="h-[50px] w-[120px] text-black transition ease-in-out hover:bg-red-900 hover:text-white hover:drop-shadow-lg">View</button></Link>
              </div>
            </div>
          )
        }
